...
ENTRYPOINT ["[ ! -f "/fancontroller/config/fan_control.yml" ] && cp /fancontroller/fan_control.yaml /fancontroller/config/fan_control.yaml;", "/fancontroller/venv/bin/python3", "-u", "/fancontroller/fan_control.py", "--config", "/fancontroller/fan_control.yaml"]
