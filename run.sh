...
echo "*** Config file..."
if [ ! -d "$TARGETDIR/fancontrol/config" ]; then mkdir $TARGETDIR/fancontrol/config fi
if [ ! -f "$TARGETDIR/fan_control.yaml" ]; then cp "/tmp/fan_control.yaml" "$TARGETDIR/config/fan_control.yaml" fi
